...
function submitForm(event) {
    ...
    const videoType = document.getElementById("search-video-type");
    const searchValue = searchInput.value.trim();
    if (searchValue !== "") {
        if(videoType.value == "movie") {
            window.location.href = "/search/movie/" + encodeURIComponent(searchValue);
            return;
        }else if(videoType.value == "series") {
            window.location.href = "/search/series/" + encodeURIComponent(searchValue);
            return;
        }
